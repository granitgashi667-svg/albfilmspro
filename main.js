// main.js - Pa Firebase, vetëm localStorage
...
function initGenresHoverWindow() {
    ...
    genresList.forEach(genre => {
        ...
        btn.onclick = () => { const targetSection = document.querySelector(`.genre-section:has(h2:contains("${genre.name}"))`); if(targetSection) targetSection.scrollIntoView({behavior:'smooth'}); else showToast(`Genre "${genre.name}" will load soon`); };
        container.appendChild(btn);
    });
}
