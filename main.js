...
function displayFavorites() {
    ...
    if(!grid) return;
    grid.innerHTML = '';
    if(favorites.length === 0){ grid.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:40px;">No favorites yet.</div>'; return; }
...
function displayWatchlist() {
    ...
    if(!grid) return;
    grid.innerHTML = '';
    if(watchlist.length === 0){ grid.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:40px;">No watch later items.</div>'; return; }
...
async function loadHeroSlider() {
    ...
    for(const m of slides){
        ...
        const rating=det.vote_average?.toFixed(1)||'N/A';
        const castList=credit.cast?credit.cast.slice(0,3).map(c=>c.name).join(', '):'N/A';
        const overview=det.overview?.substring(0,150)||'No description';
...
async function loadGenreMovies(genreId, gridId, pagId, page=1){
    ...
    try{
        const res=await fetch(`${BASE}/discover/movie?api_key=${API_KEY}&with_genres=${genreId}&sort_by=popularity.desc&vote_count.gte=100&page=${page}`);
        const data=await res.json();
...
function renderProviderGenres(){/* i njëjtë si më parë */ }
async function loadProviderContent(){/* i njëjtë */ }
function showProvider(providerName, providerId){/* i njëjtë */ }
...
// Lazy loading for genre sections
...
initSidebarSearch();
// Zhanret e zakonshme ngarkohen nga lazy observer
